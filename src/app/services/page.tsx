...
export default function Home() {
  ...
  useEffect(() => {
    getCategories(2).then((response) => setCategories(response.categories))
  }, [])
  ...
  function countRegistersFromAPI() {
    return [items.services].flat().length
  }
    ...
    <Fragment>
      ...
      <section className="flex flex-col items-center justify-center mt-8 flex-wrap gap-8">
        <section className=" flex w-full max-w-[800px] items-center space-x-2 px-7">
          <Input value={filters.search} onChange={(event) => handleUpdateSearch(event)} type="text" className="border-gray-200 h-12" placeholder="Nome do serviço..." />
      ...
      <section className="mt-8">
        <ViewControl 
          ...
          PageContent={
            <CarouselItems items={items.services} />
          }
          Fallback={
            <ItemsPageSkeleton />
          }
